...
async function main() {
    ...
    for (const item of MSWEFLS) {
        ...
        console.log('开始签到')
        let params = getSign();
        let sign = await commonPost('/JDEMaxwellApi/SignInDailyScore',{"openId":openId,"timestamp":params.time,"sign":params.sign});
        ...
        if (UserSign.state) {
            console.log(`签到获得：${UserSign.data.addWaters}水滴`)
        } else {
            console.log(UserSign.msg)
        }
        console.log("————————————")
        console.log("农场分享")
        params = getSign();
        let UserShare = await commonPost('/JDEMaxwellApi/UserShare',{"openId":openId,"timestamp":params.time,"sign":params.sign});
        if (UserShare.state) {
            console.log(`分享获得：${UserShare.data.addWaters}水滴`)
        } else {
        ...
        console.log("积分查询")
        params = getSign();
        let GetUserPoint = await commonPost('/JDEMWMall/GetUserPoint',{"openId":openId,"timestamp":params.time,"sign":params.sign});
        console.log(`拥有积分: ${GetUserPoint.data}\n`)
        notice += `用户：${id} 拥有积分: ${GetUserPoint.data}\n`
    }
    if (notice) {
        $.msg($.name, '', notice);
    }
...
async function getCookie() {
    ...
    if (index !== -1) {
        if (MSWEFLS[index].openId == newData.openId) {
            return
        } else {
            MSWEFLS[index] = newData;
            console.log(newData.openId)
            $.msg($.name, `🎉用户${newData.id}更新token成功!`, ``);
        }
    } else {
        MSWEFLS.push(newData)
        console.log(newData.openId)
        $.msg($.name, `🎉新增用户${newData.id}成功!`, ``);
    }
    $.setjson(MSWEFLS, "MSWEFLS");
}

async function commonPost(url,body = {}) {
    return new Promise(resolve => {
        const options = {
            url: `https://jde.mtbcpt.com/api${url}`,
            headers: {
                ...
                'Referer': `https://servicewechat.com/wx5b312fe433823605/117/page-frame.html`,
...
function getSign() {
    let time = Date.now();
    let sign = md5(`timestamp=${time}&openid=${openId}&key=JDEMaxwellminiapp#2021!`).toUpperCase();
