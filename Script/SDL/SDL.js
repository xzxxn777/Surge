...
async function main() {
    ...
    for (const item of SDL) {
        ...
        let save = await commonPost('/user/auth/user/collect/record/save', {"sceneValue":"1104"})
        if (save.code == 200) {
            console.log(`收藏成功，${save.data.integralToastText}`)
        } else {
...
async function getCookie() {
    ...
    if (index !== -1) {
        if (SDL[index].token == newData.token) {
            return
        } else {
            SDL[index] = newData;
