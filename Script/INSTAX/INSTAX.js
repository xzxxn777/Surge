...
async function main() {
    ...
    for (const item of INSTAX) {
        ...
        console.log("————————————")
        console.log("积分查询")
        let getPoint = await commonGet('/me');
        console.log(`拥有积分: ${getPoint.data.user.credit}\n`)
...
async function getCookie() {
    ...
    const id = body.data.user.phone_number;
    ...
    $.setjson(INSTAX, "INSTAX");
}

async function commonPost(url,body = {}) {
    return new Promise(resolve => {
        const options = {
            url: `https://instax.app.xcxd.net.cn/api${url}`,
            headers: {
