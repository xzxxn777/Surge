const $ = new Env('微信阅读')
const WxRead = ($.isNode() ? JSON.parse(process.env.WxRead) : $.getjson("WxRead")) || [];
let cookie = ''
let id = ''
let host = ''
let notice = ''
...
async function main() {
    ...
    for (const item of WxRead) {
        id = item.id;
        host = item.host;
        cookie = item.cookie;
        console.log(`用户：${id}开始任务`)
        console.log('开始签到')
        let sign = await commonPost('/yunonline/v1/sign_in',`unionid=${id}`)
        if (sign.errcode == 0) {
            console.log(`签到成功，获得${sign.data.gold}金币`)
        } else {
        ...
        console.log("开始阅读")
        while (true) {
            let wtmpdomain2 = await commonPost('/wtmpdomain2',`unionid=${id}`)
            console.log(wtmpdomain2.data.domain)
            let url = wtmpdomain2.data.domain.replace("/xyrf.html", "/jinbicp")
...
async function getCookie() {
    ...
    const newData = {"id": id, "cookie": cookie, "host": host};
    const index = WxRead.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (WxRead[index].cookie == newData.cookie) {
            return
        } else {
            WxRead[index] = newData;
