...
async function main() {
    ...
    for (const item of WuTui) {
        ...
        console.log('开始签到')
        let sign = await commonPost('/api/sign/toSign',{"uid":userId})
        if (sign.data) {
            console.log(`签到成功，获得${sign.data}`)
        } else {
            console.log(sign.msg)
        }
        let taskList = await commonPost('/api/task/getTaskList',{"uid":userId})
        ...
        console.log("————————————")
        console.log("查询积分")
        let info = await commonPost('/api/person/getUserInfoV1',{"uid":userId})
        console.log(`拥有积分：${info.data.point}\n`)
        notice += `用户：${nickname} 拥有积分: ${info.data.point}\n`
    }
    if (notice) {
        $.msg($.name, '', notice);
    }
...
async function getCookie() {
    ...
    if (!body || !body.data || !body.data.uid) {
        return
    }
    ...
    if (index !== -1) {
        if (WuTui[index].token == newData.token) {
            return
        } else {
            WuTui[index] = newData;
            console.log(newData.token)
            $.msg($.name, `🎉用户${newData.nickname}更新token成功!`, ``);
        }
    } else {
        WuTui.push(newData)
        console.log(newData.token)
        $.msg($.name, `🎉新增用户${newData.nickname}成功!`, ``);
    }
    $.setjson(WuTui, "WuTui");
}
...
async function commonPost(url,body) {
    return new Promise(resolve => {
        const options = {
            url: `https://mini-prog-wt-pro.singworld.net${url}`,
            headers : {
                ...
                'Referer': `https://servicewechat.com/wx2122a51d44658e18/381/page-frame.html`,
