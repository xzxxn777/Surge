    ...
    for (const item of MengNiuVip) {
        ...
        for (const task of list.data) {
            ...
            if (task.taskFinishStatus == 1) {
                ...
                if (task.taskKey == 'READ') {
                    let articleList = await commonPost(`/client/article/selectArticleByCategoryAndTopicTagV1102`,{"pageNo":1,"pageSize":10,"wxOpenId":wxOpenId,"categoryId":106752,"topicTagId":0,"unionId":unionId})
                    for (let i= task.taskFinishNumber; i < task.taskStage; i++) {
                        let toFinish = await commonPost(`/user/center/protein/task/toFinish?mobile=${id}&taskKey=READ`,{"unionId":unionId})
                        console.log(toFinish.msg)
                        let read = await commonPost(`/client/article/read`,{"wxOpenId":wxOpenId,"articleId":articleList.data.data[i].id,"read":1,"unionId":unionId})
                        console.log(read.msg)
                    }
                }
                if (task.taskKey == 'LIKE') {
                    let articleList = await commonPost(`/client/article/selectArticleByCategoryAndTopicTagV1102`,{"pageNo":1,"pageSize":10,"wxOpenId":wxOpenId,"categoryId":106752,"topicTagId":0,"unionId":unionId})
                    for (let i= task.taskFinishNumber; i < task.taskStage; i++) {
                        let toFinish = await commonPost(`/user/center/protein/task/toFinish?mobile=${id}&taskKey=LIKE`,{"unionId":unionId})
                        console.log(toFinish.msg)
                        let like = await commonPost(`/client/article/like`,{"articleId":articleList.data.data[i].id,"wxOpenId":wxOpenId,"like":1,"unionId":unionId})
...
async function getCookie() {
    ...
    if (index !== -1) {
        ...
        MengNiuVip.push(newData)
        console.log(newData.token)
        $.msg($.name, `🎉新增用户${newData.id}成功!`, ``);
    }
    $.setjson(MengNiuVip, "MengNiuVip");
}

async function commonPost(url,body) {
    return new Promise(resolve => {
        const options = {
            url: `https://mp-isv.youzanyun.com${url}`,
            headers : {
                ...
                'Referer': `https://servicewechat.com/wx4332719bca2e4089/122/page-frame.html`,
