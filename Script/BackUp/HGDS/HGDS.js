...
async function main() {
    ...
    for (const item of HGDS) {
        ...
        let check = await commonGet(`/check/token?idType=2&id=${unionid}&token=${token.split(' ')[1]}`,`idType=2&id=${unionid}&token=${token.split(' ')[1]}`)
        if (!check.data.isValid) {
            await sendMsg(`用户：${id}\ntoken已过期，请重新获取`);
            continue
        }
        let getMemberDetail = await commonPost('/member/getMemberDetail',{"idType":2,"id":unionid,"unionId":unionid})
        console.log(`获取用户名：${getMemberDetail.data.fullName}`)
        console.log('开始签到')
        let signInSearch = await commonGet(`/daily/signInSearch?idType=2&id=${unionid}&startTime=${getFirstDate()}&endTime=&socialHubid=${socialhubId}`,`idType=2&id=${unionid}&startTime=${getFirstDate()}&endTime=&socialHubid=${socialhubId}`)
        if (signInSearch.data.currentSignIn) {
            console.log('今日已签到')
        } else {
            let sign = await commonGet(`/daily/signIn?idType=2&id=${unionid}&socialhubId=${socialhubId}&signInDate=${getCurrentDate()}`,`idType=2&id=${unionid}&socialhubId=${socialhubId}&signInDate=${getCurrentDate()}`)
            if (sign.code == 200) {
                console.log('签到成功')
            } else {
...
async function commonGet(url, body) {
    let signArr = await getSign(body)
    return new Promise(resolve => {
        const options = {
            url: `https://hgds-crm.myaiwecom.com/api/wxapp${url}`,
            headers : {
                'Connection': 'keep-alive',
                'xweb_xhr': '1',
                'X-Hg-Req-Sign': signArr[0],
                'X-Hg-Req-Sign-Nonce': signArr[2],
                'X-Hg-Req-Sign-Ts': signArr[1],
                'Authorization': token,
                ...
                'Referer': `https://servicewechat.com/wx3656c2a2353eb377/347/page-frame.html`,
...
async function getSign(n) {
    const signTs = Date.now();
    const signNonce = sessionKey + v(20).toString()
    const secret = 'ssdjfSsFFd234ljlsSJS'
    if (typeof n === "string") {
        const jsonResult = {};
        n.split('&').forEach(pair => {
            const [key, value] = pair.split('=');
            jsonResult[key] = value;
        });
        n = jsonResult;
    }
    const queryString = Object.entries(n)
        .map(([key, value]) => {
        ...
        .sort()
        .join("&");
    const concatenatedString = `${queryString}&signTs=${signTs}&signNonce=${signNonce}&secret=${secret}`;
    const hashedString = md5(concatenatedString);
    return [hashedString, signTs, signNonce];
...
function getFirstDate() {
    ...
    return `${firstDayOfMonth.getFullYear()}-${(firstDayOfMonth.getMonth() + 1).toString().padStart(2, '0')}-01`;
}
