...
async function main() {
    ...
    for (const item of AnHuiTelecom) {
        id = item.id;
        cookie = item.cookie;
...
async function getCookie() {
    ...
    if (!body || !body.userInfo || !body.userInfo.phone) {
        return
    }
    let id = body.userInfo.phone;
    const newData = {"id": id, "cookie": cookie};
    const index = AnHuiTelecom.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (AnHuiTelecom[index].cookie == newData.cookie) {
            return
        } else {
            AnHuiTelecom[index] = newData;
            console.log(newData.cookie)
            $.msg($.name, `🎉用户${newData.id}更新cookie成功!`, ``);
        }
    } else {
        AnHuiTelecom.push(newData)
        console.log(newData.cookie)
        $.msg($.name, `🎉新增用户${newData.id}成功!`, ``);
    }
    $.setjson(AnHuiTelecom, "AnHuiTelecom");
}

async function commonGet(url) {
    return new Promise(resolve => {
        const options = {
            url: `https://wx.ah.189.cn${url}`,
            headers : {
