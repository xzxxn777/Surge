...
async function main() {
    ...
    //获取兑换列表
    if (getList) {
        let exchangeList = await commonGet("/exchange/find")
        for (const list of exchangeList.data) {
            console.log(list.name)
            for (const prize of list.exchangePrizeVoList) {
                console.log(`礼品：${prize.name} id：${prize.id} 库存：${prize.usableStock} 兑换消耗：调料包${prize.gold} 番茄${prize.score}`)
            }
        }
    }
    let success = false;
    for (let i = 0; i < 100; i++) {
        ...
        while (slide) {
            ...
            if (reward.code == 4000) {
                if (!reward.data.slideImgInfo) {
                    console.log("验证失败导致部分功能暂时用不了")
                    break
                }
                console.log("触发滑块验证")
                let data = reward.data.slideImgInfo;
