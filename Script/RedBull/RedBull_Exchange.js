...
let notice = '';
!(async () => {
    id = RedBull[RedBull_Acc].id;
    token = RedBull[RedBull_Acc].token;
    let query = await commonGet('/coupon/integral-mall/exchange-item/query?categoryId=25&pageNo=1&pageSize=10&sortKey=&sortVal=&screenExGoods=DISABLE')
    let activityNo = '';
    let itemId = '';
    for (const item of query.data.resultList) {
        console.log(`商品：${item.itemName} 需要积分：${item.exchangeIntegral}，id：${item.activityNo}`);
        if (item.activityNo == RedBull_GiftId) {
