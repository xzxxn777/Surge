...
async function main() {
    ...
    for (const item of YSHY) {
        ...
        if (signInStatus.data.signInStatus == 1) {
            ...
            let signIn = await commonPost('/signIn/save');
...
async function getCookie() {
    ...
    const index = YSHY.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (YSHY[index].token == newData.token) {
            return
        } else {
            YSHY[index] = newData;
...
async function commonPost(url,body = {}) {
    return new Promise(resolve => {
        const options = {
            url: `https://hy.51pt.top/app/ys${url}`,
            headers: {
                ...
                'Referer': `https://servicewechat.com/wx5ed5e20c00e05ba9/147/page-frame.html`,
