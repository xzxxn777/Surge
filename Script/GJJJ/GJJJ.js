/**
 * cron "19 9,17 * * *" GJJJ.js
 * export GJJJ='[{"id": "1", "openId": "1", "unionId": "1"},{"id": "2", "openId": "2", "unionId": "2"}]'
 */
const $ = new Env('顾家家居')
...
async function main() {
    ...
    for (const item of GJJJ) {
        ...
        if (sign.code == 0) {
            console.log(`签到成功，获得：${sign.data}积分`)
        } else {
            console.log(sign.message)
        }
        let articleList = await commonPost('/club-server/applet/waterfall/newWaterfall',{"pageNum":1,"pageSize":6,"source":1})
        let index = Math.floor(Math.random() * articleList.data.length);
        console.log(`文章：${articleList.data[index].title}`)
        let articleId = articleList.data[index].id
        let like = await commonPost('/club-server/front/postOrder/like',{"id":articleId})
        let likeSendPoint = await commonPost('/club-server/front/member/likeSendPoint',{"postOrderId":articleId,"triggerType":1,"content":"点赞"})
        console.log('点赞：' + likeSendPoint.message)
        let unLike = await commonPost('/club-server/front/postOrder/like',{"id":articleId})
        console.log('取消点赞：' + unLike.message)
        let collect = await commonPost('/club-server/front/postOrder/collect',{"id":articleId})
        let collectSendPoint = await commonPost('/club-server/front/member/likeSendPoint',{"postOrderId":articleId,"triggerType":2,"content":"收藏"})
        console.log('收藏：' + collectSendPoint.message)
        let uncCollect = await commonPost('/club-server/front/postOrder/collect',{"id":articleId})
        console.log('取消收藏：' + uncCollect.message)
        let share = await commonPost('/club-server/front/postOrder/share',{"id":articleId})
        let shareSendPoint = await commonPost('/club-server/front/member/likeSendPoint',{"postOrderId":articleId,"triggerType":3,"content":"微信好友转发","forwardType":2})
        console.log('分享：' + shareSendPoint.message)
        console.log("————————————")
        console.log("积分查询")
...
async function getCookie() {
    ...
    if (!body || !body.identityValue) {
        return
    }
    const id = body.identityValue;
    const openId = body.openid;
    const unionId = body.unionid;
    const newData = {"id": id, "openId": openId, "unionId": unionId};
    const index = GJJJ.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (GJJJ[index].unionId == newData.unionId && GJJJ[index].openId == newData.openId) {
            return
        } else {
            GJJJ[index] = newData;
            console.log(newData.unionId)
    ...
    return new Promise(resolve => {
        const options = {
            ...
            headers : {
                ...
                'parameterSign': params.parameterSign,
...
async function commonGet(url) {
    let params = getParams(url.split('?')[1]);
    return new Promise(resolve => {
        const options = {
            url: `https://mc.kukahome.com${url}`,
            headers : {
...
function getParams(body) {
    ...
    r = String(time).substring(4, 10);
    if (body) {
        let parameterSign = md5(md5(objToStr(body)) + r);
        return {"parameterSign": parameterSign, "sign": sign, "timestamp": time};
    } else {
        return {"parameterSign": "", "sign": sign, "timestamp": time};
    }
}

function objToStr(body) {
    let result = {};
    if (typeof body === 'string') {
        let paramsArr = body.split('&')
    ...
    return  sortedKeys.filter(key => result[key] != null && result[key] !== '').map(key => `${key}=${result[key]}`).join('&');
}
