/**
 * cron "58 59 17 25-27 * *" RQSP_Exchange.js
 * export RQSP='[{"id": "1", "phone": "1", "token": "1"},{"id": "2", "phone": "2", "token": "2"}]'
 * export RQSP_Acc="0"//兑换第一个账号
 */
const $ = new Env('日清食品体验馆-会员日盲盒兑换');
let RQSP = ($.isNode() ? JSON.parse(process.env.RQSP) : $.getjson("RQSP")) || [];
const RQSP_Acc = ($.isNode() ? process.env.RQSP_Acc : $.getdata("RQSP_Acc")) || 0;
let token='', userId = '';
!(async () => {
...
async function main() {
    ...
    userId = RQSP[RQSP_Acc].id;
    phone = RQSP[RQSP_Acc].phone;
    token = RQSP[RQSP_Acc].token;
...
async function commonGet(url) {
    return new Promise(resolve => {
        const options = {
            url: `https://prod-api.nissinfoodium.com.cn/gw-shop/app/v1${url}`,
            headers: {
                'user-id': userId,
                'ep-version': '1',
                'extra': '{"scene":1089}',
