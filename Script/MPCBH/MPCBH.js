...
async function main() {
    ...
    for (const item of MPCBH) {
        id = item.id;
        token = item.token;
        console.log(`用户：${id}开始任务`)
        console.log('开始签到')
        let sign = await commonPost('/FlanSignInDaily/adds',{"date":getCurrentDate()})
        if (sign.data.is_first == 1) {
            console.log(`签到成功，获得${sign.data.point_today}积分`)
        } else {
            console.log('今日已签到')
        }
        console.log('————————————')
        ...
        notice += `用户：${id} 拥有积分: ${userCenterStats.data.num_point}\n`
    }
    if (notice) {
        $.msg($.name, '', notice);
    }
...
async function getCookie() {
    ...
    const index = MPCBH.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (MPCBH[index].token == newData.token) {
            return
        } else {
            MPCBH[index] = newData;
...
async function commonPost(url,body) {
    ...
    return new Promise(resolve => {
        const options = {
            url: `https://mpb.jingjiu.com/proxy-he/api${url}`,
            headers : {
                ...
                'Referer': `https://servicewechat.com/wxefd0fe341e06b815/319/page-frame.html`,
...
function getSign(body) {
    ...
    let s = ''
    let i = "DYSHJS^M&.YXZRGS"
    for (const [key, value] of Object.entries(body)) {
        s += `${key}${value}`;
    }
    let sign = md5(time + s + i + token).toUpperCase().substr(-10);
