...
async function main() {
    ...
    for (const item of JDFLS) {
        ...
        chance = await commonPost(`/lottery/v1/lottery/chance/get`,{"lotteryId":lotteryId});
        for (let i = 0; i < chance.data.free; i++) {
            let lottery = await commonPost(`/lottery/v1/record/create`,{"lotteryId":lotteryId,"userName":id,"userPhone":id})
            console.log(`抽奖获得：${lottery.data.prizeName}`)
        }
        console.log("————————————")
        console.log("积分查询")
...
async function getCookie() {
    ...
    const id = body.data.memberDetail.phone;
    const memberId = body.data.memberDetail.memberId;
    const newData = {"id": id, "memberId": memberId, "projectId": projectId, "token": token};
    const index = JDFLS.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (JDFLS[index].token == newData.token) {
            return
        } else {
            JDFLS[index] = newData;
