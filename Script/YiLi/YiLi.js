/**
 * cron "5 0,18 * * *" YiLi.js
 * export YiLi='[{"id": "1", "token": "1"},{"id": "2", "token": "2"}]'
 */
const $ = new Env('伊利-摇一摇')
...
async function main() {
    ...
    for (const item of YiLi) {
        ...
        let qryUserInfo = await commonPost('/qryUserInfo',{})
        if (qryUserInfo.data.isShare == 0) {
            let share = await commonPost('/task',{"type":1})
            console.log(`分享：${share.msg}`)
        }
        if (qryUserInfo.data.isBrowse == 0) {
        ...
        if (!teamId) {
            teamId = qryTeam.data[0].id
        }
        let joinTeam = await commonPost('/joinTeam',{"teamId": teamId})
        console.log(`加入小队：${joinTeam.msg}`)
        qryUserInfo = await commonPost('/qryUserInfo',{})
        console.log(`拥有摇一摇：${qryUserInfo.data.residueShakeNum}次`)
        for (let i = 0; i < qryUserInfo.data.residueShakeNum; i++) {
            let startGame = await commonPost('/startGame',{})
            let getKey = await commonPost('/getKey',{})
            let aesKey = getKey.data + 'CD4476E9';
            let data = {"data": {"id": startGame.data, "gameScore": getRandomInt(90,105)}};
            let encryptGame = await commonPost('/encryptGame',{"decrypt": aesEncrypt(data, aesKey)})
            console.log(`抽奖获得：${encryptGame.data.awardName}`)
            notice += `用户：${id} 抽奖获得：${encryptGame.data.awardName}\n`
        }
    }
    if (notice) {
...
async function getCookie() {
    ...
    if (!body.data || !body.data.mobilePhone) {
        return
    }
    const id = body.data.mobilePhone;
    const newData = {"id": id, "token": token};
    const index = YiLi.findIndex(e => e.id == newData.id);
    if (index !== -1) {
...
async function commonPost(url,body) {
    return new Promise(resolve => {
        const options = {
            url: `https://wx-camp-wzry.mscampapi.digitalyili.com/wx-camp-exercise/stage${url}`,
            headers : {
                ...
                'Referer': `https://servicewechat.com/wx06af0ef532292cd3/501/page-frame.html`,
