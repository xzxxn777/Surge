...
async function main() {
    ...
    for (const item of YiLi) {
        ...
        let authorize = await commonGet(`/developer/oauth2/buyer/authorize?app_key=zdcade261b48eb4c5e`)
        let inputCode = await commonGet(`/fragment/ticket/input-code?code=${YiLi_Code}&authorizationCode=${authorize.data}&openId=${openId}`)
        ...
        for (let card of cardInfo.data) {
            ...
            if (card.num > 0 && YiLi_Open) {
                ...
                notice += `用户${openId} 翻卡获得：${openPrize.data.prizeName}\n`
            }
        }
    }
    if (notice) {
...
async function getCookie() {
    ...
    const newData = {"openId": openId, "token": token};
...
function getParams() {
    ...
    let signature = md5(md5("timeStamp:"+ timestamp + "&uniqueCode:" + uniquecode + "363QQ45465465xcvdas89!safzafwa36paweoi897as8dfw8g4za78qqfd878000df8/er89b").toUpperCase()).toUpperCase();
