...
async function main() {
    ...
    for (const item of arr) {
        ...
        let init = await initGet('/web/init?client_id=10050')
...
async function keyGet() {
    return new Promise(resolve => {
        const options = {
            url: `https://92261.activity-14.m.duiba.com.cn/hdtool/index?id=266434186956361&dbnewopen&from=login&spm=92261.1.1.1`,
            headers: {
        ...
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    await $.wait(2000)
                    let key = ''
                    const cheerio = Utils.createCheerio();
                    const document = cheerio.load(data);
                    const scripts = document('script');
                    scripts.each((index, script) => {
                        const scriptContent = document(script).html();
                        if (scriptContent.includes('266434186956361')) {
                            eval(scriptContent);
                            const code = getDuibaToken.toString();
                            const regex = /var\s+key\s*=\s*'([^']+)'/;
                            const match = regex.exec(code);
                            if (match) {
                                const keyValue = match[1];
                                console.log("匹配到的key值是:", keyValue);
                                key = keyValue;
                            }
                        }
                    });
...
function getBody() {
    ...
    let body = `client_id=10050&password=${password}&phone_number=${phone_number}`;
    const str = `post%%/web/oauth/credential_auth?${body}%%${uuid}%%`;
    body = `client_id=10050&password=${encodeURIComponent(password)}&phone_number=${phone_number}`;
