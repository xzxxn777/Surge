...
let BMYX = ($.isNode() ? JSON.parse(process.env.BMYX) : $.getjson("BMYX")) || [];
let token=''
let notice = ''
!(async () => {
...
async function getCookie() {
    const requestBody = $.toStr($request.body);
    if (!requestBody) {
        return
    }
    let result = {};
    let paramsArr = requestBody.split('&')
    ...
    let responseBody = await commonPost('member_center_info',requestBody);
    const id = responseBody.data.memberInfo.phone;
    const newData = {"id": id, "token": token};
    const index = BMYX.findIndex(e => e.id == newData.id);
    if (index !== -1) {
        if (BMYX[index].token == newData.token) {
            return
        } else {
            BMYX[index] = newData;
...
async function commonPost(url,body = '') {
    return new Promise(resolve => {
        const options = {
            url: `https://min.51afa.com/module/integralApi/${url}.html?`,
            headers: {
                'act': 'do_sign',
                ...
                'referer': 'https://servicewechat.com/wx51f8cb2a7578f42f/341/page-frame.html'
            },
            body:body
        }
